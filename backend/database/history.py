...
logger = logging.getLogger("trustlens.db.history")
    ...
    async def save_scan(self, scan_data: Dict[str, Any]) -> str:
        ...
        if db_manager.is_connected and db_manager.db is not None:
            try:
                ...
                logger.warning("MongoDB insert failed, using in-memory fallback: %s", type(e).__name__)
    ...
    async def get_history(self, limit: int = 50, skip: int = 0) -> List[Dict[str, Any]]:
        if db_manager.is_connected and db_manager.db is not None:
            try:
                ...
                logger.warning("MongoDB query failed, using in-memory fallback: %s", type(e).__name__)
